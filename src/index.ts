          ...
          if (voidHTMLReg.test(literalTagName)) {
            ...
            tagPath.each(
              (childPath, i) => {
                ...
                // Some nodes don't have a loc, these nodes have a body that has the loc in them,
                // so, we reach in and grab the loc from the body instead of the "parent".
                const currentLoc = getLocFromNode(childNode, true);
                const nextLoc = getLocFromNode(node.body.body[i + 1]);
